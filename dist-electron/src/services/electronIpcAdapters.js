...
exports.ElectronSchedulerService = exports.ElectronSettingsService = exports.ElectronFavoritesService = exports.ElectronHistoryService = exports.ElectronDownloadService = exports.ElectronMetadataService = void 0;
...
class ElectronMetadataService {
    async analyze(url) {
        return window.electronAPI.metadata.analyze(url);
    }
}
exports.ElectronMetadataService = ElectronMetadataService;
...
class ElectronDownloadService {
    constructor() {
        ...
        this.progressUnsubscribe = window.electronAPI.download.onProgress((payload) => {
            const item = this.itemsCache.get(payload.id);
            if (!item) {
                return;
            }
            const nonLiveStatuses = ["paused", "canceled", "failed", "completed"];
        ...
        this.stateChangeUnsubscribe = window.electronAPI.download.onStateChange((payload) => {
            const item = this.itemsCache.get(payload.id);
            if (item) {
                ...
                const isValidTransition = (0, stateMachine_1.canTransition)(item.status, payload.status);
                ...
                if (!isValidTransition && isStaleActiveEvent) {
                    console.warn(`[ElectronDownloadService] Ignoring invalid state change for ${payload.id}: ${item.status} -> ${payload.status}`);
                    return;
                }
                const updated = {
                    ...item,
    ...
    transition(item, status, now) {
        ...
        if (startRequested || resumeRequested) {
            if (this.pendingStarts.has(item.id)) {
                ...
                if (startPromise && typeof startPromise.finally === "function") {
     ...
     */
    tick(item, _now, _speedLimit) {
        // Return cached item (updated by IPC events)
        return this.itemsCache.get(item.id) || item;
    }
}
exports.ElectronDownloadService = ElectronDownloadService;
...
exports.ElectronHistoryService = ElectronHistoryService;
// ─── FavoritesService Adapter ────────────────────────────────────────────────
class ElectronFavoritesService {
    async getAll() {
...
exports.ElectronFavoritesService = ElectronFavoritesService;
// ─── SettingsService Adapter ─────────────────────────────────────────────────
class ElectronSettingsService {
    get() {
        ...
        throw new Error("[ElectronSettingsService] Synchronous get() is not supported over IPC. " +
    ...
    update(settings) {
        ...
        throw new Error("[ElectronSettingsService] Synchronous update() is not supported over IPC. " +
            "Use updateAsync().");
    }
    async updateAsync(settings) {
        return window.electronAPI.settings.update(settings);
    }
    reset() {
        void window.electronAPI.settings.reset();
        throw new Error("[ElectronSettingsService] Synchronous reset() is not supported over IPC. " +
            "Use resetAsync().");
    }
    async resetAsync() {
...
exports.ElectronSettingsService = ElectronSettingsService;
// ─── SchedulerService Adapter ─────────────────────────────────────────────────
class ElectronSchedulerService {
    async getAll() {
    ...
    async tick(now) {
        if (!window.electronAPI?.scheduler || typeof window.electronAPI.scheduler.tick !== "function") {
            const items = await this.getAll();
...
exports.ElectronSchedulerService = ElectronSchedulerService;
//# sourceMappingURL=electronIpcAdapters.js.map